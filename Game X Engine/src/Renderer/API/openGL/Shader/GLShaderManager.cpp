...
namespace gx {
	...
	void GLShaderManager::init()
	{
		...
		sh->addShader(GX_GEOMETRY_SHADER, GLDefaultSkydomeShader[1]);
		sh->addShader(GX_FRAGMENT_SHADER, GLDefaultSkydomeShader[2]);
		...
		sh->addShader(GX_GEOMETRY_SHADER, GLDefaultGBufferShader[1]);
		sh->addShader(GX_FRAGMENT_SHADER, GLDefaultGBufferShader[2]);
		defaultShaders.push_back(sh);
		...
		sh->addShader(GX_GEOMETRY_SHADER, GLDefaultDeferredShader[1]);
		sh->addShader(GX_FRAGMENT_SHADER, GLDefaultDeferredShader[2]);
		defaultShaders.push_back(sh);
		...
		sh->addShader(GX_GEOMETRY_SHADER, GLDefaultOutlineShader[1]);
		sh->addShader(GX_FRAGMENT_SHADER, GLDefaultOutlineShader[2]);
		defaultShaders.push_back(sh);
		GXE_DEBUG("Default Outline Shader Compiled Successfully");

		sh = new GXShader();
		sh->addShader(GX_VERTEX_SHADER, GLDefaultGridShader[0]);
		sh->addShader(GX_GEOMETRY_SHADER, GLDefaultGridShader[1]);
		sh->addShader(GX_FRAGMENT_SHADER, GLDefaultGridShader[2]);
		defaultShaders.push_back(sh);
		GXE_DEBUG("Default Grid Shader Compiled Successfully");

		sh = new GXShader();
		sh->addShader(GX_VERTEX_SHADER, GLDefaultShadowMapShader[0]);
		sh->addShader(GX_GEOMETRY_SHADER, GLDefaultShadowMapShader[1]);
		sh->addShader(GX_FRAGMENT_SHADER, GLDefaultShadowMapShader[2]);
		defaultShaders.push_back(sh);
